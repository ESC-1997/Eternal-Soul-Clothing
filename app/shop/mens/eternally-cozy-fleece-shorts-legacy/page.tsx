...
function EternallyCozyFleeceShortsLegacyContent() {
  ...
  useEffect(() => {
    if (selectedColor && colorToImageMap[selectedColor] !== undefined) {
      ...
      console.log('Available colors in map:', Object.keys(colorToImageMap));
      console.log('Setting image to index:', colorToImageMap[selectedColor]);
      setSelectedImage(colorToImageMap[selectedColor]);
    } else {
      console.log('Color not found in map:', selectedColor);
      console.log('Available colors in map:', Object.keys(colorToImageMap));
    }
  }, [selectedColor]);
    ...
    const fetchProduct = async () => {
      try {
        ...
        const shorts = products.find((p: any) => 
          p.title.toLowerCase().includes('eternally cozy fleece shorts (legacy)')
        );

        if (shorts) {
          // Debug logging for variant titles
          console.log('Legacy Shorts Variants from Printify:', shorts.variants.map((v: any) => ({
            title: v.title,
          ...
          const transformedProduct: Product = {
            id: shorts.id,
            title: shorts.title,
            description: "Experience ultimate comfort with our Eternally Cozy Fleece Shorts (Legacy). These premium shorts combine style with functionality, offering premium fleece material for maximum comfort, elastic waistband with drawstring for perfect fit, side pockets for convenience, and are perfect for lounging or casual wear. Machine washable for easy care.",
            images: [
              { src: '/images/fleece_shorts_legacy/fleece_shorts_ivory.png' },      // Ivory
              { src: '/images/fleece_shorts_legacy/fleece_shorts_pepper.png' },     // Pepper
              { src: '/images/fleece_shorts_legacy/fleece_shorts_blue_jean.png' },  // Blue Jean
              { src: '/images/fleece_shorts_legacy/fleece_shorts_hydrangea.png' },  // Hydrangea
              { src: '/images/fleece_shorts_legacy/fleece_shorts_neon_ivory.png' }, // Neon Ivory
              { src: '/images/fleece_shorts_legacy/fleece_shorts_peachy.png' }      // Peachy
            ],
                ...
                {availableColors.map((color) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Explicitly set the image based on the color
